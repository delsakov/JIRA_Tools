...
def get_excel_columns():
    ...
    if not excel_columns or excel_columns == {}:
        ...
        for i in range(len(excel_columns.keys())):
            index_used = 0
            for k in excel_columns.keys():
                if excel_columns[k]['index'] > len(excel_columns.keys()):
                    if index_used == 0:
                        excel_columns[k]['index'] = i
                        index_used = 1
                    else:
                        excel_columns[k]['index'] = i + 1
                elif excel_columns[k]['index'] == i and indexes == []:
                    if index_used == 0:
                        index_used = 1
                    else:
                        excel_columns[k]['index'] += 1
                elif excel_columns[k]['index'] == i:
                    if index_used == 0:
                        excel_columns[k]['index'] = indexes.pop(0)
                        index_used = 1
                    else:
                        excel_columns[k]['index'] += 1
            if index_used == 0:
...
def select_mapping_file():
    global mapping_file
    dir_name = os.getcwd()
    mapping_file = askopenfilename(initialdir=dir_name, title="Select file", filetypes=(("Release Reporting Mapping File", ".xlsx"), ("all files", "*.*")))
...
def generate_mapping_file():
    ...
    def create_mapping_file():
        global mapping_file
        dir_name = os.getcwd()
        mapping_file = asksaveasfilename(initialdir=dir_name, title="Select file", filetypes=(("Release Reporting Mapping File", ".xlsx"), ("all files", "*.*")))
        if not mapping_file.endswith('.xlsx'):
            mapping_file = mapping_file.split('.xl')[0] + '.xlsx'
        mapping.delete(0, END)
        mapping.insert(0, mapping_file)
...
def get_values_from_ui():
    ...
    global JIRA_BASE_URL, Bitbucket_URL, mapping_file, excel_columns, username, password, skip_bitbucket_flag
    
    if skip_bitbucket_flag == 0:
    ...
    JIRA_BASE_URL = jira_url.get().strip().strip('/')
...
def change_skip_bitbucket(*args):
    global skip_bitbucket_flag
    skip_bitbucket_flag = skip_bitbucket.get()
...
def main_program():
    ...
    global process_aditional_repo_flag, output_excel, process_mapping_flag, skip_bitbucket_flag, issues
    ...
    if skip_bitbucket_flag == 0 and process_aditional_repo_flag == 1 and additional_repositories:
        for i in additional_repositories:
            if i['project_name'] != '' and i['repo_names'] != '':
                if 'release_branch_name' in i and i['release_branch_name'] != '':
                    prod_branch_val = i['release_branch_name']
                else:
                    prod_branch_val = prod_branch
                if 'prod_branch' in i and i['prod_branch'] != '':
                    release_branch_name_val = i['prod_branch']
                else:
                    release_branch_name_val = release_branch_name
                opt_issues = calculate_changelog(i['project_name'], i['repo_names'].replace(' ', '').split(','), prod_branch_val, release_branch_name_val, username, password)
                new_issues = {}
...
def calculate_changelog(project_name, repo_names, prod_branch, release_branch_name, username, password):
    ...
    try:
        ...
        try:
            ...
            print("[ERROR] Connection to BitBucket failed. Skipped...")
            return {}
    
    print("[INFO] Issues associated with Commits in {} / {} are being downloaded from BitBucket...".format(project_name, repo_names))
    ...
    print("[INFO] Issues for {} / {} have been successfully downloaded from BitBucket.".format(project_name, repo_names))
    
    return issues


def update_epics(key):
    global epic_details, jira
    try:
        epic = jira.issue(key)
        ep = [key, epic.fields.summary]
...
def updated_stories(param):
    global jira, updated_issues, excel_columns, process_mapping_flag
    
    (key, k, len_details) = param
    details = ['' for i in range(len_details)]
    try:
        try:
            issue = jira.issue(key)
        except:
            print("ERROR: Issue '{}' can't be opened".format(key))
            return (1, param)
        
        for field_name in excel_columns.keys():
            try:
                if field_name == 'ID':
                    details[excel_columns[field_name]['index']] = key
                ...
                print("Exception '{}' for retrieving JIRA details for JIRA_ID: {}".format(e, key))
...
def get_jira_metadata(issues):
    global updated_issues, all_issues, repositories, excel_columns, fields_ids_mapping, custom_ad_field, epic_details
    global max_retries, default_max_retries, skip_bitbucket_flag
    ...
    if skip_bitbucket_flag == 0:
        ...
        print("Metadata for Issues for {} Sheet is being downloaded from JIRA...".format(repositories))
    
    for k, v in issues.items():
        print("Total JIRA issues for '{}' to be processed: {}".format(k, len(v)))
        
        len_details = len(excel_columns)
        params = [(i, k, len_details) for i in v]
        max_retries = default_max_retries
        threads_processing(updated_stories, params)
        
        if len(v) > 0:
            print("Processed {} issues for {} list.".format(len(v), k))

    if skip_bitbucket_flag == 0:
        print("Metadata for Issues in {} repositories has been successfully downloaded from JIRA.".format(repositories))
    else:
        print("Metadata for Issues for {} Sheet has been successfully downloaded from JIRA.".format(repositories))
    ...
    if excel_columns['Epic Name']['visible'] == 1:
        ...
        if skip_bitbucket_flag == 0:
            ...
            print("Metadata for Epics for {} Sheet is being downloaded from JIRA...".format(repositories))
        
        # Retrieve Epic details from JIRA
        print("Total Epics/ Parents to be processed: {}".format(len(epics)))
        params = [e for e in epics]
        max_retries = default_max_retries
        threads_processing(update_epics, params)
        ...
        if skip_bitbucket_flag == 0:
            ...
            print("Metadata for Epics for {} Sheet has been successfully downloaded from JIRA.".format(repositories))
...
skip_bitbucket = IntVar(value=skip_bitbucket_flag)
Checkbutton(main, text="skip Bitbucket section", font=("Helvetica", 9, "italic"), variable=skip_bitbucket).grid(row=0, sticky=E, padx=20, columnspan=4, pady=0)
skip_bitbucket.trace('w', change_skip_bitbucket)
